...
export const getVentasActivos = async (req, res) => {
  ...
  export const addNumEnsambleVinil = async (req, res) => {
    try {
      const pool = await getConnection();
        const result = await pool
        .request()
        .input("id", req.params.id)
        .input("EQBRAND_codEnsamble", req.body.Factura)
        .query(querys.addNumEnsambleVinil);
        if(result.rowsAffected==1){
            return res.status(200).json({ status: "ok", msg: "Actualizacion exitosa" ,token:0});
          }else{
            return res.status(400).json({ status: "400", msg: "No se pudo actualizar, consulte al administrador" ,token:0});
  ...
  export const addNumEnsambleVinilEquipo = async (req, res) => {
    try {
      const pool = await getConnection();
        const result = await pool
        .request()
  ...
  export const addNumEnsambleEquipo = async (req, res) => {
    try {
      const pool = await getConnection();
        const result = await pool
        .request()
